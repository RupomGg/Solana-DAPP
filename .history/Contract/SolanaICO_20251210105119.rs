...
declare_id!("94AijGUGuKoxmEKiNFVnCApRJSDDBbXMVXFTFqLF8jfJ"); // it will generate automatically in Solona playground
...
pub mod ico {
    ...
    pub const ICO_MINT_ADDRESS: &str = "GG7fp1Unue93bFXE5sNMZz7yFvGFhbQKgVKBWUF6RzH9";
    ...
    pub fn create_token_metadata(
        ...
        msg!("Creating token metadata for {} ({})", name, symbol);
    ...
    pub fn deposite_ico_in_ata(ctx: Context<DepositeIcoATA>, ico_amount: u64) -> Result<()> {
        ...
        msg!(
            "Deposited {} Additional ICO tokens to Program ATA",
            ico_amount
        );
        Ok(())
    }
    ...
    pub fn buy_tokens(
        ...
        _ico_ico_ata_for_ico_program_bump: u8,
        token_amount: u64,
    ) -> Result<()> {
        ...
        let ix = anchor_lang::solana_program::system_instruction::transfer(
            &ctx.accounts.user.key(), // ctx means context which actually stand here from user public key , its a short from
            &ctx.accounts.admin.key(), // here means to admin(owner) public key
            sol_amount,
        );
        ...
        let seeds = &[
            ...
            &[_ico_ico_ata_for_ico_program_bump],
        ]; // A bump is a tiny number (0-255) that pushes the address off the elliptic curve so it doesn't have a private key.
